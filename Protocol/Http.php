...
class Http implements \Server\Protocol\TcpProtocolInterface
{
    public static function input($buff, TcpConnection $connection)
    {
        ...
        } elseif (strpos($headerData, "GET") === 0) {
...
//          var_dump($v);
    ...
    {
        $header = isset(HttpCache::$header["httpCapital"])
            ? HttpCache::$header["httpCapital"]."\r\n" : "HTTP/1.1 200 Ok\r\n";
        $header .= isset(HttpCache::$header["Content-Type"])
            ? HttpCache::$header["Content-Type"]."\r\n" : "Content-Type: text/html; charset=UTF-8\r\n";
        foreach (HttpCache::$header as $key => $v) {
            if (strtolower($key) == "set-cookie") {
                foreach ($v as $v2) {
                    $header .= $v2."\r\n";
                }
            } elseif ($key != "httpCapital" && $key != "Content-Type") {
                $header .= $v."\r\n";
            }
        }
    ...
    public static function sessionStart()
    {
        ...
        if (!HttpCache::$sessionFile) {
            HttpCache::$sessionFile = HttpCache::$sessionPath
                ."/".HttpCache::$sessionFilePrefix.$_COOKIE[HttpCache::$sessionName];
        }
